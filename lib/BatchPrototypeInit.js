(function (undefined) {
    ...
    var judges = {
        ...
        aspectResize: decree(defs.args.resize.slice(0, -1)),
        ...
        toBase64JpgEncodedFormData: decree(defs.args.toBase64JpgEncodedFormData),
    ...
    Batch.prototype.aspectResize = function () {
        var that = this;
        judges.aspectResize(arguments, function (width, height, inter) {
            that.__addOp(that.__image.aspectResize, [width, height, inter].filter(undefinedFilter));
        });
        return this;
    };
    ...
    Batch.prototype.toBase64JpgEncodedFormData = function () {
        var that = this;
        judges.toBase64JpgEncodedFormData(arguments, function (callback){
            that.exec(function (err, image) {
                if (err) return callback(err);
                image.toBase64JpgEncodedFormData(callback);
            });
        })
    };
