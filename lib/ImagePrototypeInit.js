(function (undefined) {
    ...
    var judges = {
        ...
        aspectResize: decree(defs.args.resize),
    ...
    Image.prototype.aspectResize = function () {
        ...
        judges.aspectResize(
            ...
            function (width, height, inter, callback) {
                height = height || width;
                var size = that.scaledSize(width, height);
                that.__lwip.resize(+size.w, +size.h, defs.interpolations[inter], function (err) {
                    that.__release();
                    callback(err, that);
                });
            },
    ...
    Image.prototype.toBase64JpgEncodedFormData = function () {
        var that = this;
        judges.toBase64JpgEncodedFormData(
            arguments,
            function (callback) {
                that.toBuffer('jpg', function (err, buffer) {
                    if (err) return callback(err);
